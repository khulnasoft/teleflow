...
const mockConfig = {
  ...
  from: 'test@teleflow.khulnasoft.com',
  senderName: 'test',
};
...
test('should correctly use sender email and name from the config', async () => {
  ...
  const { from, ...mockTeleflowMessageWithoutFrom } = mockTeleflowMessage;

  // use config.from if message.from is not provided
  await provider.sendMessage(mockTeleflowMessageWithoutFrom);
  expect(spy).toHaveBeenCalled();

  // Use the message.from instead of config.from if available
  const res = await provider.sendMessage(mockTeleflowMessage);
  expect(spy).toHaveBeenCalled();
